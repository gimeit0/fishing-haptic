...
✅ 単体デモ（8状態の釣りシミュレータ、LCD+IMU+ブザー、main.cpp）
✅ アタリ波形（10Hz + 23Hz 合成、バースト+ランダム間隔）の実装 → src/haptics.cpp
✅ 引き波形（5ms 高 + T ms 低 非対称矩形波、DC フリー）の実装 → src/haptics.cpp
✅ IMU で合わせ動作検出（BITE 中 |a|>1.5g）・上提検出（FIGHTING 中 gyro スパイク）
✅ 状態遷移ロジック完成（NIBBLE/BITE→アタリ振動、FIGHTING→引き波形に連動）
⬜ 実機で加振器の振動確認・振幅/パルス間隔のチューニング（予備評価）

### 触覚チューニング（シリアルコマンド 115200bps）

- `t 0|n|b|p` : モード強制 (off/nibble/bite/pull)、`t g` でゲーム連動に戻す
- `pa <0-1>` : 引き振幅スケール
- `pt <ms>` : パルス間隔 T を固定（`pt 0` で自動 = 魚の引きに連動 12–22ms）
- `na <0-1>` : アタリ振幅
