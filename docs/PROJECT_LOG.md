...
### 2026-08-26 (六) — 「走らせる」模式实装后撤回(commit c1ea14d → revert)

- 曾实装延べ竿实钓循环「ため⇄走らせる」(`rn 1` 开关):突进中鱼出线、
  竿放低也持续振动;顶着不放=打滑+1s 断线;让竿=轻退+跑鱼计时冻结。
- **实测撤回**:该机制下突进段无论让竿与否都持续接近预算上限的输出
  (旧行为里让竿=静音,供电轨得以喘息),突进期间总电流反而上升,
  加重了 brownout。发表前以稳定优先,整体 revert。
- 代码保留在 git 历史(`c1ea14d`),硬件供电对策(电容/独立 5V)落地后
  可直接 revert-the-revert 复活,届时该机制顺带修复"突进时恰好放竿
  会漏掉最强尺寸线索段"的问题。
