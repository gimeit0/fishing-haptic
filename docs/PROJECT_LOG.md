...
### 2026-08-26 (六) — 「走らせる」模式先行实装(`rn 1`,默认关)

- 发表展望的先行实装:把延べ竿实钓的「ため ⇄ 走らせる」循环带进 HOLD——
  **突进中鱼在出线、ライン张り続ける** → 竿放低也持续振动(顺带修复:
  旧行为里突进时恰好放竿会漏掉最强的尺寸线索段);
  **顶着突进不放** = 泄力打滑(复用 click train)+ 进度 0.6× 倒退,
  连续保持 1s(`RUN_HOLD_BREAK_MS`)断线——短走可硬扛,长走必须让;
  **放低竿"让"**(正解)= 进度仅 0.2× 轻退,跑鱼计时冻结;
  突进开始以 480Hz 低音警告(区别于高音系),画面提示 FISH RUNS!/let it run。
- **默认 OFF**(`rn 0|1`),不改变感知实验的刺激条件;演示时开。
- 已烧录:2号机(1/3号机为上一版,无 rn 命令)。

### 2026-08-26 (五) — 尺寸对比度改为"小鱼往下让":供电预算内恢复弁別度

- 思路(用户提出):尺寸弁別是**相对**判断——与其让 BIG 顶供电天花板,
  不如把 SML 往下让,在 `pl 0.60` 预算内做宽对比。旧映射下两条件的突进段
  都被钳到 0.60 削平(那一瞬手感相同),对比度实际已受损。
- 改动:`TEN_AMP_MIN` 0.55→**0.40**(SML 振幅更低,SML 突进 ≈0.57 收进
  预算内不再被削);`TEN_T_BIAS_SML` -3→**-5ms**(SML 上探 40–60Hz 贴共振点,
  机械效率补偿低振幅的存在感,频率差拉大且不增电流);段完成瞬态余振
  τ 50–150ms(钳位下 tap 振幅两条件同一,尺寸差改由余振时长承担)。
  BIG 侧参数不动(29Hz 低频=重 保留)。
- 待实机确认:SML 持续段绝对强度是否仍清晰可感(过弱则 `fs` 上调水准)。
- 已烧录:2号机。

### 2026-08-26 (四) — BIG 条件 brownout 重启对策:引き峰值限幅器 `pl`

- 实机问题:BIG 条件战斗中黑屏重启。原因=供电 brownout:满幅(钳位1.0)×
  29Hz(共振点以下阻抗最小≈Re 4Ω)×3.2s 连续 → MAX98357A(4Ω/5V 可达 3.2W)
  产生 ~1A 电流尖峰,M5StickS3 的 5V 引脚(M5PM1 供电路)被拉垮。
  官方 docs 亦警告电池供电时内置 SPK 音量 >75% 即可重启。
- 固件对策:PULL 最终振幅默认钳位 **0.75**(`PULL_PEAK_LIMIT_DEF`),新串口
  命令 `pl <0-1>` 调整。只削满幅突进段,BIG/SML 持续振幅比不变。
- **追记(实测复现后二次修正)**:串口日志显示板子撑过 3.2s 持续振动,
  死在 `SEGMENT 1/4 done` 瞬间——基线限幅漏掉了段完成/突进的满幅瞬态敲击
...
### 2026-08-26 (三) — 待机触觉改为 25s 心跳,连续底流默认停用

- 实机反馈:待机时 28Hz 连续底流"一直在微微震,很奇怪"。
- `HAPTIC_WAVE` 重做:45s 渚涌浪 + 连续底流 → **每 25s(±10%)一次心跳双跳**
  "トク・トクン"(lub 120ms + 180ms + dub 110ms×0.75,sin² 包络),
  振幅默认降至 0.20。25s < 判停窗口(30–60s),保活由心跳兼任。
- 底流合成保留但默认 0(`wk` 可复活);`wa`/`wi` 语义不变,默认 0.20/25s。
- 若充电宝判停复发(电源监视变橙 BAT):`wa`↑ 或 `wi`↓,再不行 `wk 0.35`。
- 已编译烧录至 COM3。

### 2026-08-26 (二) — PUMP 废止,大小鱼二择感知实验版(分支 hold-mode-v3)

- 研究问题聚焦:**只凭触觉能否弁別鱼的大小**。FIGHTING 只保留张力保持(HOLD)
  玩法;BtnB/`fm` 从"玩法切换"变为 **SML/BIG 条件切换**,fishSize01 由随机
  抽签改为固定水准(SML=0.20/BIG=0.85,`fs`/`fb` 可调)。
- **排除结构线索**:段数/段长默认两条件同一(4 段×3.2s,`dc 1` 恢复尺寸联动)
  ——否则"打得久=大鱼"会淹没触觉通道。尺寸差异只在:振幅 0.64 vs 0.94、
  基频 ~45Hz vs ~33Hz、段完成瞬态 τ 76 vs 128ms。
- **盲测模式 `bl 1`**:隐藏屏幕全部尺寸线索(标签/鱼种/A%T 调试行)和
  BtnB 切换音差;串口日志(cond=BIG size=... blind=1)作为实验者的正解记录。
- 删除:泵竿检测、甜区光标渲染、运动耦合(`mc`)、相关常量与全局变量。
  鱼行为模型(突进/休息/疲劳)保留——继续驱动 HOLD 的振幅纹理与事件。
- 台架测试 `t p` 现在也带尺寸差与节律,`fm 0/1`+`t p` 可直接对比两刺激。
- 已编译烧录至 COM3。详见 `docs/changes/2026-08-26-大小鱼二择感知实验版.md`。
